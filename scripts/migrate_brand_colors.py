...
def get_or_create_brand_config(
    ...
    """
    existing = (
        db.query(BrandConfig)
        .filter(BrandConfig.tenant_id == tenant_id)
        .first()
    )
...
def migrate_brand_colors(db: Session | None = None) -> dict:
    ...
    try:
        ...
        for config in RIVERSIDE_BRAND_CONFIGS:
            ...
            # Exact match
            if brand_name.lower() in tenant_map:
                tenant = tenant_map[brand_name.lower()]
            # Partial match for "Lash Lounge"
            elif "lash" in brand_name.lower():
                for name, t in tenant_map.items():
                    if "lash" in name:
                        tenant = t
                        break
            # Partial match for "Frenchies"
            elif "french" in brand_name.lower():
                for name, t in tenant_map.items():
                    if "french" in name:
                        tenant = t
                        break
            # Partial match for "Bishops"
            elif "bishop" in brand_name.lower():
                for name, t in tenant_map.items():
                    if "bishop" in name:
                        tenant = t
                        break
            # Partial match for "HTT"
            elif brand_name.lower() == "htt":
                for name, t in tenant_map.items():
                    if "htt" in name or "head" in name:
                        tenant = t
                        break
            
            if not tenant:
                logger.warning(
                    f"No matching tenant found for brand: {brand_name}. Skipping."
                )
            ...
            try:
                ...
                existing = (
                    db.query(BrandConfig)
                    .filter(BrandConfig.tenant_id == tenant.id)
                    .first()
                )

...
class AzureCostAnalyzer:
    ...
    def _get_cost_mgmt_client(self) -> Any:
        ...
        if self._cost_mgmt_client is None:
            try:
                ...
                logger.error("Azure SDK not installed. Run: pip install azure-mgmt-costmanagement azure-identity")
                raise
    ...
    def _get_detailed_costs_sdk(
        ...
        for row in result.rows:
            ...
            breakdowns.append(CostBreakdown(
                ...
                usage_quantity=Decimal(str(row[quantity_idx])) if quantity_idx else Decimal("0"),
    ...
    def _get_detailed_costs_cli(
        ...
        cmd = [
            ...
            "--time-period", f"to={end_date.strftime('%Y-%m-%d')}",
            "--dataset-aggregation", "totalCost=PreTaxCost",
            "--dataset-grouping", "ResourceGroup",
            "--dataset-grouping", "ServiceName",
            "--output", "json"
        ]
        ...
        try:
            ...
            for resource_type in self.IDLE_PRONE_RESOURCES:
                ...
                for resource in resources:
                    idle_info = self._check_resource_idle(
                        resource, monitor_client, days_threshold
                    )
                    if idle_info:
    ...
    def _check_resource_idle(
        ...
        try:
            ...
            metric_name = "Percentage CPU" if resource_type == "Microsoft.Compute/virtualMachines" else "Transactions"
            ...
            if resource_type == "Microsoft.Compute/virtualMachines":
                if avg_usage < 5:  # Less than 5% CPU average
                    is_idle = True
                    reason = f"Very low CPU usage: {avg_usage:.1f}% average over {days_threshold} days"
    ...
    def check_cost_thresholds(
        self,
        threshold_monthly: Decimal,
        threshold_daily: Decimal | None = None
    ) -> list[CostAlert]:
    ...
    def generate_cost_report(
        ...
        days: int = 30,
        group_by_tenant: bool = False,
        include_idle_resources: bool = False
    ) -> dict[str, Any]:
        ...
        for c in costs:
            by_resource_group[c.resource_group] = by_resource_group.get(c.resource_group, Decimal("0")) + c.cost_usd
        ...
        return {
            ...
            "by_service": {k: float(v) for k, v in sorted(by_service.items(), key=lambda x: x[1], reverse=True)},
            "by_resource_group": {k: float(v) for k, v in sorted(by_resource_group.items(), key=lambda x: x[1], reverse=True)},
            "by_tenant": {k: float(v) for k, v in sorted(by_tenant.items(), key=lambda x: x[1], reverse=True)} if by_tenant else {},
    ...
    def export_to_csv(self, report: dict[str, Any], output_path: str) -> None:
        ...
        with open(output_path, "w", newline="") as f:
            ...
            if report["idle_resources"]:
                ...
                writer.writerow(["Resource Name", "Type", "Resource Group", "Monthly Cost", "Recommendation"])
    ...
    def send_alert_notifications(self, webhook_url: str | None = None) -> bool:
        ...
        webhook = webhook_url or os.environ.get("TEAMS_WEBHOOK_URL") or os.environ.get("SLACK_WEBHOOK_URL")
        ...
        try:
            ...
            if "office.com" in webhook or "teams" in webhook:
                ...
                payload = {
                    ...
                    "themeColor": "ff0000" if any(a.severity == "critical" for a in self._alerts) else "ff9900",
...
Examples:
  ...
  %(prog)s --subscription $SUB_ID --export-csv costs.csv
    ...
    report = analyzer.generate_cost_report(
        days=args.days,
        group_by_tenant=args.by_tenant,
        include_idle_resources=args.find_idle
    )

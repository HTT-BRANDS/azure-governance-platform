...
class KeyVaultSecretRotator:
    ...
    def _get_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                logger.error("Azure Key Vault SDK not installed. Run: pip install azure-keyvault-secrets azure-identity")
                raise
    ...
    def verify_soft_delete(self) -> bool:
        ...
        try:
            ...
            if not subscription_id or not resource_group:
                logger.warning("Cannot verify soft-delete: AZURE_SUBSCRIPTION_ID or AZURE_RESOURCE_GROUP not set")
            ...
            if not soft_delete:
                ...
                logger.error("Secret rotation without soft-delete is dangerous - deleted secrets cannot be recovered!")
            else:
    ...
    def generate_secure_value(self, secret_type: str = "password", length: int = 32) -> str:
    ...
    def rotate_secret(
        ...
        try:
            ...
            if dry_run:
                logger.info(f"[DRY RUN] Would rotate {secret_name} (current version: {previous_version})")
                return RotationResult(
                    secret_name=secret_name,
                    success=True,
            ...
            logger.info(f"✅ Successfully rotated {secret_name} (new version: {new_secret.properties.version})")
...
Examples:
  ...
  %(prog)s --vault my-kv --secret my-secret --generate --dry-run
    ...
    parser.add_argument("--type", "-t", default="password", 
    ...
    parser.add_argument("--no-soft-delete-check", action="store_true", help="Skip soft-delete verification")

...
class ThemeService:
    ...
    def get_css_variables(self, tenant_id: str) -> dict[str, str]:
        ...
        css_vars["--brand-primary-text"] = self._get_contrasting_text(
            theme["primary_color"]
        )
        css_vars["--brand-secondary-text"] = self._get_contrasting_text(
            theme["secondary_color"]
        )
        
        if theme.get("accent_color"):
            css_vars["--brand-accent-text"] = self._get_contrasting_text(
                theme["accent_color"]
            )
        else:

...
class ServiceHealthIncident:
    ...
    def is_platform_impacting(self) -> bool:
        ...
        platform_services = [
            "App Service", "Azure SQL", "Storage", "Key Vault",
            "Azure AD", "Microsoft Graph", "Azure Resource Manager",
...
class AzureServiceHealthMonitor:
    ...
    def _get_service_health_client(self) -> Any:
        ...
        if self._service_health_client is None:
            try:
                ...
                self._service_health_client = MicrosoftResourceHealth(credential, self.subscription_id)
            except ImportError:
                logger.error("Azure SDK not installed. Run: pip install azure-mgmt-resourcehealth azure-identity")
                raise
    ...
    def _get_resource_health_client(self) -> Any:
        ...
        if self._resource_health_client is None:
            try:
                ...
                credential = DefaultAzureCredential()
                self._resource_health_client = MicrosoftResourceHealth(credential, self.subscription_id)
            except ImportError:
    ...
    def get_active_incidents(self) -> list[ServiceHealthIncident]:
        ...
        try:
            from azure.mgmt.resourcehealth.models import EventStatusValues
    ...
    def _convert_event_to_incident(self, event: Any) -> ServiceHealthIncident | None:
        ...
        try:
            severity = IncidentSeverity(event.severity) if hasattr(event, 'severity') else IncidentSeverity.WARNING
            status = IncidentStatus(event.status) if hasattr(event, 'status') else IncidentStatus.ACTIVE
            ...
            return ServiceHealthIncident(
                ...
                last_update_time=event.last_modified_time if hasattr(event, 'last_modified_time') else datetime.utcnow(),
    ...
    def check_resource_impact(self, resource_id: str) -> ResourceHealthEvent | None:
        ...
        try:
            ...
            availability = client.availability_statuses.get_by_resource(
    ...
    def check_platform_impact(self) -> dict[str, Any]:
        ...
        for service_name, service_type in platform_services.items():
            ...
            service_incidents = [
                ...
                if service_name.lower() in i.service.lower() or 
                   service_type.lower() in i.service.lower()
            ]
    ...
    def register_notification_handler(self, handler: Callable[[ServiceHealthIncident], None]) -> None:
    ...
    async def _send_teams_notification(self, incident: ServiceHealthIncident) -> bool:
        ...
        try:
            ...
            color = "ff0000" if incident.severity == IncidentSeverity.CRITICAL else "ff9900" if incident.severity == IncidentSeverity.ERROR else "0099ff"
                ...
                "sections": [
                    {
                        ...
                        "facts": [
                            ...
                            {"name": "Started", "value": incident.start_time.strftime("%Y-%m-%d %H:%M UTC")},
                            {"name": "Platform Impact", "value": "Yes" if incident.is_platform_impacting else "No"},
                        ],
                ...
                "potentialAction": [
                    {
                        ...
                        "targets": [
                            {"os": "default", "uri": f"https://portal.azure.com/#blade/Microsoft_Azure_Health/HealthHistoryBlade"}
                        ]
                    }
                ]
            }
    ...
    async def _send_slack_notification(self, incident: ServiceHealthIncident) -> bool:
        ...
        try:
            ...
            emoji = "🔴" if incident.severity == IncidentSeverity.CRITICAL else "🟡" if incident.severity == IncidentSeverity.ERROR else "🔵"
            ...
            message = {
                ...
                "blocks": [
                    ...
                    },
                    {
                        "type": "context",
                        "elements": [
                            {"type": "mrkdwn", "text": f"Started: {incident.start_time.strftime('%Y-%m-%d %H:%M UTC')}"}
                        ]
                    }
                ]
            }
        ...
        while not self._shutdown:
            try:
                ...
                new_incidents = [
                    ...
                    if i.incident_id not in self._active_incidents
                ]
                
                # Find resolved incidents
                resolved_ids = set(self._active_incidents.keys()) - {i.incident_id for i in current_incidents}
                
                # Notify about new incidents
                for incident in new_incidents:
                    if incident.is_platform_impacting or incident.severity in (IncidentSeverity.CRITICAL, IncidentSeverity.ERROR):
                        logger.warning(f"New platform-impacting incident detected: {incident.title}")
    ...
    def generate_health_report(self, days: int = 30) -> dict[str, Any]:
        ...
        avg_resolution = sum(i.duration_minutes for i in resolved) / len(resolved) if resolved else 0

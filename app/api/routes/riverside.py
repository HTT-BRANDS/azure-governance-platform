...
router = APIRouter(tags=["riverside"])
templates = Jinja2Templates(directory="app/templates")


@router.get("/riverside", response_class=HTMLResponse)
async def riverside_dashboard(request: Request, db: Session = Depends(get_db)):
...
async def get_riverside_summary(db: Session = Depends(get_db)):
...
async def get_maturity_scores(db: Session = Depends(get_db)):
